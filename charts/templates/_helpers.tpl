...
{{define "basePod" -}}{{if hasKey .template "restartPolicy"}}{{if (lt 0 (len .template.restartPolicy)) -}}
restartPolicy: {{.template.restartPolicy}}
{{end}}{{end}}{{if hasKey .template "imagePullSecrets"}}{{if (lt 0 (len .template.imagePullSecrets)) -}}
imagePullSecrets: {{include "imagePullSecrets" .template.imagePullSecrets | nindent 0}}
{{end}}{{end}}{{if hasKey .template "initContainers"}}{{if (lt 0 (len .template.initContainers)) -}}
initContainers: {{include "containers" (dict "Values" .Values "containers" .template.initContainers) | nindent 0}}
{{end}}{{end}}{{if hasKey .template "containers"}}{{if (lt 0 (len .template.containers)) -}}
containers: {{include "containers" (dict "Values" .Values "containers" .template.containers) | nindent 0}}
{{end}}{{end}}{{end}}
